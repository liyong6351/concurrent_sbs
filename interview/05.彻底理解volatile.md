...
- [volatile](#volatile)
  - [1. volatile简介](#1-volatile%E7%AE%80%E4%BB%8B)
  - [2. volatile实现原理](#2-volatile%E5%AE%9E%E7%8E%B0%E5%8E%9F%E7%90%86)
  - [3. volatile的happens-before关系](#3-volatile%E7%9A%84happens-before%E5%85%B3%E7%B3%BB)
  - [4. volatile的内存语义](#4-volatile%E7%9A%84%E5%86%85%E5%AD%98%E8%AF%AD%E4%B9%89)
    - [4.1 volatile的内存语义实现](#41-volatile%E7%9A%84%E5%86%85%E5%AD%98%E8%AF%AD%E4%B9%89%E5%AE%9E%E7%8E%B0)
  - [5. 一个示例](#5-%E4%B8%80%E4%B8%AA%E7%A4%BA%E4%BE%8B)
...
![](images/5VolatileExample的happens-before关系推导.png)
...
![mem](images/5内存屏障分类表.png)
java编译器会在生成指令系列时在适当的位置会插入内存屏障指令来禁止特定类型的处理器重排序。为了实现volatile的内存语义，JMM会限制特定类型的编译器和处理器重排序，JMM会针对编译器制定volatile重排序规则表：
![](images/5volatile重排序规则表.png)
java编译器会在生成指令系列时在适当的位置会插入内存屏障指令来禁止特定类型的处理器重排序。为了实现volatile的内存语义，JMM会限制特定类型的编译器和处理器重排序，JMM会针对编译器制定volatile重排序规则表：
...
![order](images/5volatile写插入内存屏障示意图.png)
![order](images/5volatile读插入内存屏障示意图.png)
